...
from bom_processor import get_processed_lcsc_bom
import os
...
bom = get_processed_lcsc_bom(bom_path)
...
for index, row in bom.iterrows():
    ...
    entry["Distributor Part Number"] = bom.loc[index, 'LCSC Part Number']
    entry["Quantity"] = int(bom.loc[index, 'Order Qty.'])
    entry["Description"] = bom.loc[index, 'Description']

    # Footprint from loaded BOM is matched to predefined list
    if bom.isna().loc[index, 'Package']:
        entry.pop("Footprint", None)
    else:
        entry["Footprint"] = bom.loc[index, 'Package']
